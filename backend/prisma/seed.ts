import { PrismaClient, SkillType, SkillLevel, SwapStatus, CourseVisibility, PriceType, ChatMessageType } from '@prisma/client';
...
async function main() {
  ...
  await prisma.userSkill.createMany({
    data: [
      { userId: mentorUser.id, skillId: guitarSkill.id, skillType: SkillType.TAUGHT },
      { userId: mentorUser.id, skillId: kotlinSkill.id, skillType: SkillType.WANTED },
    ],
  });
  ...
  await prisma.userSkill.createMany({
    data: [
      { userId: learnerUser.id, skillId: kotlinSkill.id, skillType: SkillType.TAUGHT },
      { userId: learnerUser.id, skillId: guitarSkill.id, skillType: SkillType.WANTED },
    ],
  });
  ...
  const course = await prisma.course.create({
    data: {
      ...
      priceType: PriceType.FREE_WITH_SWAP,
      priceAmount: 0.00,
      duration: '2h 15m',
      rating: 4.95,
      reviewCount: 38,
