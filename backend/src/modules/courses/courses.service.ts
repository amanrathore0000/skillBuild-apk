...
export class CoursesService {
  async getCourses(query: { categoryId?: string; search?: string; level?: string }) {
    const where: any = {
  ...
  async getCourseById(courseId: string, userId?: string) {
    ...
    const lessons = course.lessons.map((l) => {
      const userProg = (l as any).progress && (l as any).progress.length > 0 ? (l as any).progress[0] : null;
      return {
        id: l.id,
        title: l.title,
        duration: l.duration,
  ...
  async createCourse(mentorId: string, data: {
    title: string;
    description: string;
    categoryId: string;
    duration?: string;
  ...
  async addLesson(courseId: string, mentorId: string, data: {
    title: string;
    duration: string;
  ...
  async syncLessonProgress(userId: string, lessonId: string, data: { watchedSeconds: number; isCompleted?: boolean }) {
  ...
  async getStoragePlans(userId: string) {
    const plans = [
      {
        id: 'plan_gdrive_free',
    ...
    const subscription = await (prisma as any).storageSubscription?.findFirst({
  ...
  async subscribeStoragePlan(userId: string, planId: string) {
    const plans: Record<string, { limit: bigint; provider: 'AWS_S3' | 'GOOGLE_DRIVE' }> = {
    ...
    if ((prisma as any).storageSubscription) {
      await (prisma as any).storageSubscription.updateMany({
        where: { userId },
        data: { isActive: false },
      });

      const subscription = await (prisma as any).storageSubscription.create({
        data: {
          userId,
          planId,
  ...
  async grantDriveAccess(userId: string, lessonId: string, learnerEmail: string) {
    const lesson = await prisma.lesson.findUnique({
      where: { id: lessonId },
    });
    ...
    return {
      success: true,
      lessonId,
      driveFileId: (lesson as any).driveFileId || `drive_${lesson.id}`,
      learnerEmail,
  ...
  async generateUploadPresignedUrl(userId: string, fileType: string, fileName: string, storageProvider: 'AWS_S3' | 'GOOGLE_DRIVE' = 'AWS_S3') {
